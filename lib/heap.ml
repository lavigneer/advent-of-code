(* Taken from https://ocaml.org/manual/5.2/api/Dynarray.html#example_min_heap *)
...
module Heap (Elem : Map.OrderedType) : sig
  ...
  and heap_down h ~len i =
    ...
    if left >= len then () (* no child, stop *)
    else
      let smallest =
        if right >= len then left (* no right child *)
        else if order h left right < 0 then left
        else right
      in
